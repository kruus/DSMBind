    ...
    def __init__(self, data_path, cdr_type, epitope_size):
        ...
        with open(data_path) as f:
            for line in tqdm(f.readlines()):
                ...
                entry["scaffold_idx"] = idx = [
                    i
                    for i, v in enumerate(entry["antibody_cdr"])
                    if v not in "0" + cdr_type
                ]
                ...
                entry["target_coords"] = torch.cat(
                    (entry["scaffold_coords"], entry["epitope_coords"]), dim=0
                )
                entry["target_atypes"] = torch.cat(
                    (entry["scaffold_atypes"], entry["epitope_atypes"]), dim=0
                )
                entry["target_dihedrals"] = torch.cat(
                    (entry["scaffold_dihedrals"], entry["epitope_dihedrals"]), dim=0
                )
    ...
    def make_decoy_batch(batch, K=0):
        ...
        for ab in batch:
            ...
            for _ in range(K):
                ...
                ab["paratope_seq"] = "".join(
                    [random.choice(ALPHABET[1:]) for _ in ab["paratope_seq"]]
                )
                ab["paratope_atypes"] = torch.tensor(
                    [
                        [ATOM_TYPES.index(a) for a in RES_ATOM14[ALPHABET.index(s)]]
                        for s in ab["paratope_seq"]
                    ]
                )
                new_batch.append(ab)
