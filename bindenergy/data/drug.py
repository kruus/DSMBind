...
class DrugDataset:
    ...
    def __init__(self, data_path, patch_size):
        ...
        for entry in tqdm(data):
            ...
            entry["binder_coords"] = torch.tensor(
                [[p.x, p.y, p.z] for p in coords]
            ).float()
            # make pocket
            dist = entry["target_coords"][:, 1] - entry["binder_coords"].mean(
                dim=0, keepdims=True
            )
            entry["pocket_idx"] = idx = (
                dist.norm(dim=-1).sort().indices[:patch_size].sort().values
            )
            entry["pocket_seq"] = "".join(
