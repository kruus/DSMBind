...
from bindenergy.utils.utils import _density, _expansion, _score
...
class DrugEnergyModel(FARigidModel):
    ...
    def __init__(self, args):
        ...
        self.W_o = nn.Sequential(nn.SiLU(), nn.Linear(args.hidden_size, 1))
        self.theta_range = np.linspace(0.1, np.pi, 100)
        self.sigma_range = np.linspace(0, 10.0, 100) + 0.1
        self.expansion = [
            _expansion(self.theta_range, sigma) for sigma in self.sigma_range
        ]
        self.density = [_density(exp, self.theta_range) for exp in self.expansion]
        self.score = [
            _score(exp, self.theta_range, sigma)
            for exp, sigma in zip(self.expansion, self.sigma_range)
        ]
    ...
    def forward(self, binder, target):
        ...
        B, _, _ = bind_S.size(0), bind_S.size(1), tgt_X.size(1)
        ...
        f = torch.autograd.grad(
            energy.sum(), bind_X, create_graph=True, retain_graph=True
        )[0]
        f = f[:, :, 1]  # force [B,N,3]
        ...
        Inertia = self.inertia(bind_X[:, :, 1], bind_mask)  # [B,3,3] inertia matrix
        w = torch.linalg.solve(Inertia.detach(), G)  # angular velocity

        # Score matching loss
        score = (
            torch.tensor([self.score[i][j] for i, j in zip(sidx, tidx)]).float().cuda()
        )
    ...
    def predict(self, binder, target):
        ...
        # B, N, M = bind_S.size(0), bind_S.size(1), tgt_X.size(1)
        # _, _, _ = bind_S.size(0), bind_S.size(1), tgt_X.size(1)
        bind_mask = bind_A[:, :, 1].clamp(max=1).float()
        tgt_mask = tgt_A[:, :, 1].clamp(max=1).float()
        mask = torch.cat([bind_mask, tgt_mask], dim=1)
        mask_2D = mask.unsqueeze(2) * mask.unsqueeze(1)
...
class DrugAllAtomEnergyModel(FARigidModel):
    ...
    def sc_rotate(self, X, w):
        ...
        cross_wX = torch.cross(w, X, dim=-1)
        return (X
                + c1 * cross_wX
                + c2 * torch.cross(w, cross_wX)
                )
    ...
    def forward(self, binder, target, use_sidechain=True):
        ...
        B, N, M = bind_S.size(0), bind_S.size(1), tgt_X.size(1)
        bind_mask = bind_A[:, :, 1].clamp(max=1).float()
        # tgt_mask = tgt_A[:, :, 1].clamp(max=1).float()          # unused
        ...
        # sigma = torch.tensor([self.sigma_range[i] for i in aidx]).float().cuda()  # unused
        ...
        # Contact map
        mask_2D = (bind_A > 0).float().view(B, N * 14, 1) * (tgt_A > 0).float().view(
            B, 1, M * 14
        )
        D = (bind_X.view(B, N * 14, 1, 3) - tgt_X.view(B, 1, M * 14, 3)).norm(
            dim=-1
        )  # [B,N*14,M*14]
        mask_2D = mask_2D * (D < self.threshold).float()
        # Energy
        ...
        # force
        f_bind, f_tgt = torch.autograd.grad(
            energy.sum(), [bind_X, tgt_X], create_graph=True, retain_graph=True
        )
        ...
        Inertia = self.inertia(tgt_X, sc_mask)  # [B*N,3,3] inertia matrix
        Inertia = Inertia + self.eye * (1 - has_sc)[:, None, None]  # avoid zero inertia
        u = torch.linalg.solve(Inertia.detach(), G)  # [B*N, 3] angular velocity
        ...
        score = (
            torch.tensor([self.score[i][j] for i, j in zip(aidx, bidx)]).float().cuda()
        )
    ...
    def virtual_screen(self, protein_pdb, sdf_list, batch_size=200):
        ...
        for ligand_sdf in sdf_list:
            df = PandasTools.LoadSDF(
                ligand_sdf, molColName="Molecule", includeFingerprints=False
            )
            mol = df["Molecule"][0]
            entry = {
                "binder_mol": mol,
                "target_seq": hseq,
                "target_coords": hcoords,
            }
    ...
    def gaussian_forward(self, binder, target):
        ...
        B, N, M = bind_S.size(0), bind_S.size(1), tgt_X.size(1)
        # bind_mask = bind_A[:, :, 1].clamp(max=1).float()        # unused
        # tgt_mask = tgt_A[:, :, 1].clamp(max=1).float()          # unused
        bind_A = bind_A * (true_X.norm(dim=-1) > 1e-4).long()
        tgt_A = tgt_A * (tgt_X.norm(dim=-1) > 1e-4).long()
...
class DrugDecoyModel(DrugAllAtomEnergyModel):
    ...
    def forward(self, binder, target, num_decoy=1):
        ...
        BB = B * (num_decoy + 1)
        # Contact map
        mask_2D = (bind_A > 0).float().view(BB, N * 14, 1) * (tgt_A > 0).float().view(
            BB, 1, M * 14
        )
        D = (bind_X.view(BB, N * 14, 1, 3) - tgt_X.view(BB, 1, M * 14, 3)).norm(
            dim=-1
        )  # [B,N*14,M*14]
        mask_2D = mask_2D * (D < self.threshold).float()
        # Energy
        ...
        energy = (energy * mask_2D).sum(dim=(1, 2))  # [B]
        energy = torch.stack(
            [energy[B * i: B * i + B] for i in range(num_decoy + 1)], dim=-1
        )  # [B, K+1]
